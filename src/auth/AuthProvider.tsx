...
export function AuthProvider({ children }: { children: React.ReactNode }) {
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      if (!active) return;
      if (error) {
        console.error("[AuthProvider] profiles fetch error", {
          error,
          status,
          userId: user.id,
        });
      ...
      if (!data) {
        ...
        setTimeout(async () => {
          ...
          if (!active) return;
          if (e2) {
            console.error("[AuthProvider] profiles retry error", {
              e2,
              userId: user.id,
            });
          } else if (d2?.role) {
  ...
  const effectiveRole =
    profileRole || metaRole || (allowlistAdmin ? "admin" : null);
