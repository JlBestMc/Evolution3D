...
export default function Navbar({
  ...
  return (
    <>
      <div
        className={`hidden md:flex ${bgColor} flex-row justify-between items-center p-4 px-16 text-black-300`}
      >
        ...
        >
          ...
          <a onClick={() => navigate("/museum")} className={aStyles}>
            Museum
          </a>
        </div>
            ...
            <>
              {isAdmin && (
                <Button2
                  ...
                  gradientHover="from-cyan-500 to-blue-500"
          ...
          {open && (
            <div className="absolute left-0 right-0 mt-2 rounded-2xl p-2 backdrop-blur-xl bg-gray-800 border border-white/15 shadow-[0_8px_30px_rgba(0,0,0,0.25)]">
              <div className="flex flex-col">
                ...
                <button
                  onClick={() => handleNavigate(PATHS.timeline)}
                  className="text-left px-3 py-2 rounded-xl text-sm font-medium text-white/85 hover:text-white hover:bg-white/10"
                >
                  Timeline
                </button>
                <button
                  onClick={() => handleNavigate(PATHS.museum)}
                  className="text-left px-3 py-2 rounded-xl text-sm font-medium text-white/85 hover:text-white hover:bg-white/10"
                >
                  Museum
                </button>
                <div className="px-2 pt-1 flex gap-2">
                  {user ? (
                    <>
                      ...
                      <Button2 onClick={handleLogout} rounded="rounded-full">
                        Logout
                      </Button2>
                    </>
                  ) : (
                    <>
                      <Button2
                        onClick={() => handleNavigate(PATHS.login)}
                        rounded="rounded-full"
                      >
                        Login
                      </Button2>
