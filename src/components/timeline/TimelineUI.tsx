      ...
      <div className="absolute bottom-10 left-1/2 -translate-x-1/2 flex flex-col items-center gap-5 select-none z-20">
        ...
        >
          ...
          <div className="relative text-center text-white drop-shadow flex flex-col items-center px-4">
            <h2 className="text-2xl font-semibold tracking-wide leading-tight mb-1">
              {era.name}
            </h2>
            {era.period && (
              ...
              <button
                ...
                className={`w-3.5 h-3.5 rounded-full border transition-all ${
                  i === index
                    ...
                    : "border-white/40 bg-white/10 hover:bg-white/40"
                }`}
                aria-label={e.name}
              />
            ))}
        ...
        <div className="mt-6 w-[88vw] max-w-[900px] h-24 relative">
          {/* Línea principal */}
          ...
          <div className="absolute left-0 right-0 top-[calc(50%+6px)] flex justify-between px-4">
            {Array.from({ length: 40 }).map((_, i) => (
              <span
                key={i}
                className="w-[2px] h-[2px] bg-white/50 rounded-full opacity-50"
              />
            ))}
          </div>
          ...
          <div className="absolute inset-0 flex items-center justify-between px-6">
            {eras.map((e, i) => (
              <div key={e.id} className="relative w-0 h-0">
                ...
                <button
                  onClick={() => !loading && setCurrentEra(e.id)}
                  className={`absolute -top-16 left-1/2 -translate-x-1/2 w-14 h-10 rounded-md overflow-hidden ring-1 transition-all ${
                    i === index
                      ? "ring-white/80 shadow-md scale-105"
                ...
                <button
                  aria-label={e.name}
                  onClick={() => !loading && setCurrentEra(e.id)}
                  className={`relative w-4 h-4 rounded-full border transition-all ml-[-8px] ${
                    i === index
                      ? "bg-white border-white shadow-[0_0_12px_rgba(255,255,255,0.8)] scale-110"
                      : "bg-white/20 border-white/60 hover:bg-white/50"
                  }`}
                />
